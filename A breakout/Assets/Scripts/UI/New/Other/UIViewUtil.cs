...
namespace EasyFrameWork.UI
{
    ...
    public class UIViewUtil : Singleton<UIViewUtil>
    {
        ...
        private void ShowWindowCenterToBig(UIWindowViewBase window, bool isOpen)
        {
            ...
            {
                window.transform.localScale = Vector3.zero;
                window.transform.DoScale(Vector3.one, window.Duration);
            }
            else
            {
                window.transform.localScale = Vector3.one;
                window.transform.DoScale(Vector3.zero, window.Duration).SetEndAction(() => { DestoryWindow(window); });
            }
        ...
        private void ShowWindoweFromDir(UIWindowViewBase window, int dirIndex, bool isOpen)
        {
            ...
            ((RectTransform)(window.transform)).anchoredPosition = from;
            ((RectTransform)(window.transform)).DoMove(to, window.Duration).SetIsLocal(true).SetEndAction(() =>
            {
                if (!isOpen)
                {
                    DestoryWindow(window);
