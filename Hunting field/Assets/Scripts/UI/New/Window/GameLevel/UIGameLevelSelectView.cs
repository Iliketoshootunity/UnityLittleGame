...
namespace EasyFrameWork
{
    public class UIGameLevelSelectView : UIWindowViewBase
    {
        ...
        [SerializeField]
        private int m_Count = 12;
        [SerializeField]
        private int m_Count2 = 4;
        ...
        private IEnumerator ColumnAniamtion(int index1, int index2)
        {
            ...
            {
                if (m_LevelViewList[j] != null && m_LevelViewList[j].gameObject.activeSelf)
                {
                    Tween t = m_LevelViewList[j].transform.DoScale(Vector3.one, m_ScaleTime).SetAnimatorCurve(m_ScaleCurve).SetIsSpeed(false);
                    m_TweenList.Add(t);
